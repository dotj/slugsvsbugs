...
uint8_t CheckLBeacon(void) {
    static uint8_t lastBeaconStateL = 0;
    uint8_t currentBeaconStateL = 0;
    uint8_t eventDetected = FALSE;

    currentBeaconStateL = Bot_CheckLBeacon();

    if(currentBeaconStateL != lastBeaconStateL) {
        ES_Event ThisEvent;
        ThisEvent.EventType = BEACONLEFT;
        ThisEvent.EventParam = currentBeaconStateL;
        PostBotFSM(ThisEvent);
        eventDetected = TRUE;
    }

    lastBeaconStateL = currentBeaconStateL;
    return eventDetected;
}

uint8_t CheckRBeacon(void) {
    static uint8_t lastBeaconStateR = 0;
    uint8_t currentBeaconStateR = 0;
    uint8_t eventDetected = FALSE;

    currentBeaconStateR = Bot_CheckRBeacon();

    if(currentBeaconStateR != lastBeaconStateR) {
        ES_Event ThisEvent;
        ThisEvent.EventType = BEACONRIGHT;
        ThisEvent.EventParam = currentBeaconStateR;
        PostBotFSM(ThisEvent);
        eventDetected = TRUE;
    }

    lastBeaconStateR = currentBeaconStateR;
    return eventDetected;
}
