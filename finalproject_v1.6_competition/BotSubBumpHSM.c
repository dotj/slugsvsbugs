        ...
        case Arc1:
            switch (ThisEvent.EventType) {
                case ES_ENTRY:
                    ...
                    if (compass==WEST) {
                        SpinRight(10);
                        printf("using compass, initial turn right\n");
                    } else if (compass==EAST) {
                        SpinLeft(10);
                        printf("using compass, initial turn left\n");
                    } else {
                    ...
                    ES_Timer_InitTimer(BUMPSTATES_TIMEOUT, ARC1_DURATION);
                    break;
                case ES_EXIT:
                    break;
                case TAPECHANGE:
                ...
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == BUMPSTATES_TIMEOUT) {
                        ...
                        ES_Timer_InitTimer(BUMPSTATES_TIMEOUT, MOVE1_DURATION);   // timer to move straight
                        nextState = Move;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                        FullStop();
        ...
        case Arc2:
            switch (ThisEvent.EventType) {
                case ES_ENTRY:
                    ...
                    ES_Timer_InitTimer(BUMPSTATES_TIMEOUT, ARC2_DURATION);
                    break;
                case ES_EXIT:
                    break;
                case TAPECHANGE:
...
//                    break;
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == BUMPSTATES_TIMEOUT) {
                        switch(arcCount) {
                            case 1: nextState = Arc1; break;
                            case 2: nextState = Arc2; break;
                            case 3: nextState = Arc3; break;
                            case 4: nextState = Arc4; break;
                            default: break;
                        }
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
